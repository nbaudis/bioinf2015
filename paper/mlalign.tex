...
The values for $\overline{p_n}$ can be pre-computed in constant time.
...
To foster competition among the teams, an award (dinner payed by Alexis) was announced for the team that would implement the fastest code. 
...
Static analyses help to identify static programming errors such as incorrect programming language syntax or typing errors.
We compiled our codes with the \texttt{gcc} compiler using {\em all} available and reasonable warning flags\footnote{-Wall -Wextra -Wredundant-decls -Wswitch-default -Wimport -Wno-int-to-pointer-cast -Wbad-function-cast -Wmissing-declarations -Wmissing-prototypes -Wnested-externs -Wstrict-prototypes -Wformat-nonliteral -Wundef}. 
...
periodically alongside of \texttt{gcc} to further reduce the amount of potential programming errors. Note that, \verb|clang| conducts a static code analysis.

\subsubsection{Dynamic Analysis}
These analyses cover run-time issues, mainly memory leaks.
...
Section~\ref{ssec:dataalignment} outlines our considerations regarding the data alignment that constitutes an important part of the vectorized implementation. 
...
Green and red cells outside of the matrix boundaries represent the offset calculation for the intermediate and closing part, respectively. 
For the DP calculations, only indices that correspond to
DPM entries are required. 

Given the anti-diagonal indexing scheme for a single matrix, we now need to 
...
or partial loads (i.e., load single elements into vectors).
...
The penalties functions $C^i$ that can be stored in lookup tables take one or two nucleotide states as parameters.
Thus, the penalties can be stored in a memoization table of $4$ and $16$ entries, respectively. 
...
However, the evaluation of the resulting performance revealed that too much time is spent to populate the table. Thus, we abandoned this approach.
...
This does not degrade performance, since we did not observe a significant performance degradation when
...
When vectorizing along the anti-diagonals, accessing the memory in the DPMs when they are stored in a row-, or column-major scheme 
...
Therefore, we used the aforementioned anti-diagonalized matrix layout together with a struct of arrays approach (see section~\ref{ssec:memorylayout}), 
such that unaligned \texttt{load} and \texttt{store} operations for copying data directly from the anti-diagonal 
...
\begin{figure}[ht!]
  ...
  \caption{Outline of the data required for calculating an anti-diagonal}
  \label{fig:alignment}
\end{figure}

In Figure~\ref{fig:alignment}, we show the required values for computing the anti-diagonal elements  with AVX intrinsics (vector length: $4$ double precision values). 
We want to calculate the red elements in the current step. 
...
We did not observe any numerical issues for the range of input parameters values and sequence lengths (up to and including $10,000$ nucleotides) 
we tested.
...
A major disadvantage of logarithmic transformations is that errors in the logarithm computation accumulate over a series of operations. 
...
We denote the code obtained by applying the above techniques and transformations the basic version.
...
the entries of a single inbdex for all three matrices. The structs are stored contiguously in row-major order.}
\label{fig:aos}
\end{figure}
...
However, this slowed down the program as computing the mapped indices required more arithmetic operations than needed for the actual TKF91 algorithm.
...
by using the \texttt{\_\_attribute\_\_(aligned(\textit{size}))} attribute for stack allocations, 
...
would result in memory accesses at incorrectly aligned adresses. 
...
we had to apply several modifications to be able to use vector intrinsics in conjunction with the more cache-efficient data structure.
...
However, a single  \textit{\_mm\_load\_pd} on contiguous memory is substantially faster than a series of \textit{\_mm\_loadl\_pd} and \textit{\_mm\_loadh\_pd} operations
...
Our log-space version {\bf which one of the two?} of TKF91 reduces the DP cell update to determining the largest of two or three values. 
In turn, this means that the relative amount of time spent for DP cell updates is negligible. 
...
Thus, merely indexing a cell, using the standard row and column coordinates, requires a too large amount of computations. 
Our preliminary benchmarks yielded absolutely no performance improvement for the vectorized versions of 
the basic as well as the improved version of our code.
...
We found that the median of five samples, where each sample consisted of the average run-time for ten \textit{iterations} 
(i.e., $5*10*45 = 2250$ pair-wise alignments per data set {\bf data-set consistent!}) 
yielded stable run time estimates. 
...
Thereafter, we measure the run-time {\bf runtimes, run-times, run times} of our code in Section~\ref{team2-runtimes}.
...
It includes versions of the logarithm function, which allow to explicitely specify the type of rounding strategy ({\bf for instance?}) to be used.

To assess the correctness of our alignments, we computed the edit distance between the alignments {\bf how is the edit distance between alignemnts defined? This needs to be explained!} 
...
In particular the \texttt{log\_ru} function {\bf where does this come from?}, which explicitly rounds up, produces a notably differnt alignment compared to 
\texttt{Boost.Multiprecision} reference alignment (see Table~\ref{fig:dist}). 
{\bf the following sentence is inconsisten, first you say notably different and then highly similar?} 
...
As input sequences for measuring run times,  we used four pairs of randomly generated sequences with $10$, $100$, $1000$, and $10000$ nucleotides. 
...
We executed the kernel multiple times for each dataset and subsequently averaged the run times. 
...
\includegraphics[width=\textwidth]{images/benchplot.png}
    \caption{Comparison of average run times of different implementations. \texttt{sse3} and \texttt{avx} post-fixes denote the vectorized versions of the programs, 
